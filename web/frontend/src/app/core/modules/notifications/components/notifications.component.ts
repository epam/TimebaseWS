...
export class NotificationsComponent implements OnDestroy, OnInit {
  ...
  ngOnInit() {
    ...
    this.notificationsState.pipe(takeUntil(this.destroy$), delay(0)).subscribe(() => this.cdRef.detectChanges());
  }
  ...
  onClosed(notification: NotificationModel, index: number) {
    ...
    } else if (notification.requestDialogParams && notification.requestDialogParams.closeActions && notification.requestDialogParams.closeActions.onCancel) {
