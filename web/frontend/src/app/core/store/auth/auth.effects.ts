...
export class AuthEffects {
  @Effect({dispatch: false}) getAuthProviderInfo = this.actions$.pipe(
    ...
    tap((resp) => {
      this.appStore.dispatch(
        new AuthActions.SetAuthProviderSettings({
  ...
  @Effect({dispatch: false}) loadSSOConfig = this.actions$.pipe(
    ...
    switchMap((/*action*/) => {
      return this.appStore.pipe(select(getSSOConfiguration), take(1));
    }),
    switchMap((config) => {
      if (config != null) {
        return of({type: 'none'});
          ...
          return this.httpClient
            ...
            .pipe(
              tap((providerConfigJSON) => {
                this.appStore.dispatch(
                  new AuthActions.SaveSSOConfig({
                    configuration: new AuthorizationServiceConfiguration(providerConfigJSON),
                  }),
                );
              }),
            );
  ...
  @Effect({dispatch: false}) silentUpdateSSOToken = this.actions$.pipe(
    ...
    switchMap((provider: AuthProviderModel) =>
      this.appStore.pipe(
    ...
    tap(
      ([[config, appProviderConfig], isTokenInitialized]: [
  ...
  private setNotLoggedIn(): Observable<boolean> {
    return this.localStorage.getItem(AUTH_KEY_IN_LS).pipe(
      catchError((e) => {
