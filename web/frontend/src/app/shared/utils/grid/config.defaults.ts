  ...
  return [
    ...
    // 'resetColumns',
    {
      ...
      action: function () {
        ...
        copyToClipboard(JSON.stringify(params.node.data[type])).pipe(take(1)).subscribe();
      },
    },
    'separator',
    {
      ...
      subMenu: [
        ...
        },
        {
          ...
          action: () => params.api.exportDataAsExcel({...params, exportMode: 'xml', processCellCallback}),
        },
      ],
    },
  ];
...
export function columnIsPinned(pinnedEvent: ColumnPinnedEvent, tabName: string, gridStateLS: GridStateModel) {
  if ((GridContextMenuService.executing || pinnedEvent.source === 'uiColumnDragged') && pinnedEvent.column) {
  ...
  if (localStorage.getItem('gridStateLS' + tabName)) {
    ...
    if (allColumns && allColumns.length) {
      ...
      if (positionUpdates && Object.keys(positionUpdates).length) {
        ...
        colState.filter(c => !newStateIds.has(c.colId)).forEach(col => {
...
export function columnsVisibleColumn(withChild = true): ColGroupDef | ColDef {
  const cell = {headerName: '', colId: 'GridHeaderGlobalMenuComponent', pinned: true, maxWidth: 30, width: 30, filter: false,  lockPosition: true};
  if (!withChild) {
