              ...
              *ngFor="let control of form.get(field.name)['controls']; let i = index"
              ...
              <ng-template ngFor let-field [ngForOf]="this.field.childFields">
                <app-field-builder
                  ...
                  [field]="field" [form]="control"></app-field-builder>
              </ng-template>
              <div class="btn-panel">
                ...
                <ng-template [ngIf]="(form.get(field.name)['controls'].length > 1 && field.required) ||
...
export class ArrayComponent implements OnInit {
  ...
  get isValid() { return this.form.get(this.field.name) ? this.form.get(this.field.name).valid : true; }
  
  get isDirty() { return this.form.get(this.field.name) ? this.form.get(this.field.name).dirty : false; }
  ...
  ngOnInit(): void {
    ...
    const controls = this.field.values && this.field.values.length ? this.field.values : this.field.required ? [{}] : [];
    this.controlsArray = new FormArray(controls.map(this.getFormGroup.bind(this)), (this.field.required ? [Validators.required] : null));
  ...
  getFormGroup(value: any) {
    ...
    this.field.childFields.forEach((field: FieldModel) => {
      controls[field.name] = new FormControl(value[field.name] !== null ? value[field.name] : '', (field.required ? [Validators.required] : null));
    });
