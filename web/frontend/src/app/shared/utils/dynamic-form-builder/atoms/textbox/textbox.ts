    ...
    <div [formGroup]="form"
      ...
      <ng-template [ngIf]="field.upload_file">
        ...
        <div class="d-flex btns-wr">
          <input #uploadInput
            type="file" class="upload-control" (change)="onUpload($event)" tabindex="-1">

          <button
            class="btn btn-primary btn-upload"
            type="button"
            tabindex="0"
            (click)="uploadInput.click()">{{'buttons.upload' | translate}}</button>
        </div>
      </ng-template>
    </div>
...
export class TextBoxComponent implements OnInit {
  ...
  onUpload(event: Event) {
    ...
    this.translate.get('notification_messages')
      .pipe(
        switchMap((messages: string) => {

          const formData = new FormData();
          let url = `connectors/${this.form['_connectorType']}/stunnel/cert/upload/${this.form.get('name').value}`;
