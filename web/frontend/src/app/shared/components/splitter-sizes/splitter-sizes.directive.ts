...
export class SplitterSizesDirective implements AfterContentInit, OnDestroy, OnChanges {
  ...
  ngAfterContentInit(): void {
    ...
    this.splitComponent.dragStart.pipe(takeUntil(this.destroy$)).subscribe(() => this.dragging = true);
    this.splitComponent.dragEnd.pipe(
      takeUntil(this.destroy$),
    ).subscribe(({sizes}) => {
      this.lastSizes = sizes as number[];
      this.dragging = false;
      if (this.storageKey) {
        this.getStorageValue().pipe(
          switchMap(value => {
            const data = value || {};
    ...
    this.resizeObserveService.observe(this.elementRef.nativeElement).pipe(
      takeUntil(this.destroy$),
    ).subscribe(() => {
      this.recountPercents();
    });
    
    const childResizes = Array.from(this.elementRef.nativeElement.children)
      .map(child => this.resizeObserveService.observe(child as HTMLElement));
    
    merge(...childResizes).pipe(
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.storageKey) {
      this.getSizes().pipe(take(1)).subscribe(sizes => {
  ...
  private recountPercents() {
    const minSizes = this.minSizes.map(
      (size, index) => Math.max(this.pixelToPercent(size), this.pixelToPercent(this.childMinWidth[index] || 0)),
    );
    
    this.minWidth = this.minSizes
      .reduce((acc, s, index) => {
    ...
    if (this.parentAreaIndex !== undefined) {
      this.parentSplitterSizes.setChildMinSize(this.parentAreaIndex, this.minWidth + this.parentWidthAddon);
    }
  ...
  private getSizes(): Observable<number[]> {
    return this.getStorageValue().pipe(map(value => {
