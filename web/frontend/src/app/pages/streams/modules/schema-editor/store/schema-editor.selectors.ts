...
export const getDefaultsTypes = createSelector(
  getEditSchemaState,
  (state: State) => {
...
export const getDDTypesNames = createSelector(
  getEditSchemaState,
  (state: State) => {
    return ([...state.defaultTypes.map(type => type.name), ...state.enums.map(_enum => _enum.name)]);
  },
);
...
export const getSaveSchemaData = createSelector(
  getEditSchemaState,
  (state: State) => {
    return {
...
export const getSelectedSchemaItem = createSelector(
  getAllSchemaItems,
  (allSchemaItems: SchemaClassTypeModel[]) => allSchemaItems.find(schemaItem => schemaItem._props && schemaItem._props._isSelected),
);

export const getSelectedSchemaItemFields = createSelector(
  getSelectedSchemaItem,
  (selectedSchemaItem: SchemaClassTypeModel) => selectedSchemaItem ? selectedSchemaItem.fields : [],
);
...
export const getSelectedSchemaItemAllFields = createSelector(
  ...
  getAllSchemaItems,
  (selectedSchemaItem: SchemaClassTypeModel, selectedSchemaItemFields: SchemaClassFieldModel[], allSchemaItems: SchemaClassTypeModel[]) => {
    return selectedSchemaItem ? selectedSchemaItem.isEnum ? [...selectedSchemaItemFields] : [...getParentFields(selectedSchemaItem.parent, allSchemaItems.filter(_type => !_type.isEnum)), ...selectedSchemaItemFields] : [];
  },
);
...
export const getSelectedFieldProps = createSelector(
  ...
  (allSchemaItems: SchemaClassTypeModel[]): SchemaClassFieldModel | null => {
    const SELECTED_TYPE = allSchemaItems.find(_type => _type && _type._props && _type._props._isSelected),
      SELECTED_TYPE_FIELDS = SELECTED_TYPE?.fields;

    return SELECTED_TYPE_FIELDS && SELECTED_TYPE_FIELDS.length ? SELECTED_TYPE_FIELDS.find(field => field._props._uuid === SELECTED_TYPE._props._selectedFieldUuid) : null;
  },
);
...
export const ifNewFieldIsAdding = createSelector(
  getSelectedSchemaItem,
  getSelectedSchemaItemFields,
  (selectedSchemaItem: SchemaClassTypeModel, selectedSchemaItemFields: SchemaClassFieldModel[]) => !selectedSchemaItem?.isEnum && selectedSchemaItemFields.length ? selectedSchemaItemFields.some(field => !field.name) : false,
);
