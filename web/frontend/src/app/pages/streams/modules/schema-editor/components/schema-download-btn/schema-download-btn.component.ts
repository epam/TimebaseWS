...
import { GetSchema }                             from '../../store/schema-editor.actions';
import { getAllSchemaItems, getEditSchemaState } from '../../store/schema-editor.selectors';
...
export class SchemaDownloadBtnComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.href$ = combineLatest([
      ...
      map(([data, formState]) => {
        const state = {classes: this.clearProps(data.classes), enums: this.clearProps(data.enums)};
        return this.sanitizer.bypassSecurityTrustUrl(
          `data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify({state, formState}, null, '\t'))}`,
        );
      }));
    
    this.title$ = this.appStore.pipe(select(getActiveOrFirstTab), filter(Boolean)).pipe(
      map((tab: TabModel) => `${tab.stream}.schema.json`),
    );
  }
