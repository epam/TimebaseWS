...
export class SchemaUploadBtnComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.control.valueChanges.pipe(
      ...
      switchMap(([files, messages]: [FileList, { jsonParseError: string, dataError: string }]) => {
        ...
        const result$ = fromEvent(fr, 'load').pipe(take(1), map(() => {
          try {
      ...
      filter(Boolean),
      switchMap(json => this.confirmModalService.confirm('schemaEditor.importFromJson.confirm').pipe(
        map(confirm => confirm ? json : null),
      )),
      filter(Boolean),
  ...
  private updateSchema(json) {
    (this.errorHandler as GlobalErrorHandler).catchErrors(() => {
      this.seFieldFormsService.restoreFromState(json.formState);
      this.appStore.dispatch(EditSchemaUpdateState({
      ...
      switchMap(message => this.onCloseTabAlertService.doWithoutAlert().pipe(map(() => message))),
