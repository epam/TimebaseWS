...
export class FlControlPanelComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.ifFieldSelected$ = this.selectedSchemaItem$
      .pipe(
        map(selectedItem => !!(selectedItem && selectedItem._props && typeof selectedItem._props._selectedFieldUuid === 'string')),
      );
  ...
  public onAddNewField(isStatic?: boolean) {
    ...
    this.appStore.dispatch(AddNewFieldForSelectedSchemaItem({
      isStatic,
      ...this.nameForm.value,
    }));
  ...
  public onAskDeleteSelected() {
    this.selectedField()
      .pipe(
        filter(Boolean),
        switchMap((field: SchemaClassFieldModel) => this.translate.get(`text.removeFieldRequest`, {name: field.name})),
        take(1),
  ...
  public onAskToAdd(isStatic?: boolean) {
    this.selectedSchemaItem$
      .pipe(
        take(1),
      )
      .subscribe((selectedSchemaItem) => {
        
        this.nameForm = this.fb.group({
          name: [
            null,
            selectedSchemaItem?.isEnum ? [Validators.required] : [Validators.required, Validators.pattern(FIELD_NAME_PATTER_REGEXP)],
            this.forbiddenNames(),
          ],
        });
        this.nameForm.reset();
