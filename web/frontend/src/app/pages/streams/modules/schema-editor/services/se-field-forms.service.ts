...
export class SeFieldFormsService {
  ...
  dumpState(): Observable<SeFieldFormsServiceState> {
    return combineLatest([
      this.hasAnyError$,
      this.formChanged$,
      this.typesHasChanges$,
    ]).pipe(map(([hasAnyError, formChanged, typesHasChanges]) => {
      return {
        hasAnyError,
  ...
  typesChanged(types: SchemaClassTypeModel[]) {
    ...
    types?.forEach(
      type => {
        ...
        type.fields.forEach(
          field => existingKeys.add(this.key(type._props._uuid, field._props._uuid)),
        );
      },
    );
  ...
  revertForm(): void {
    combineLatest([this.currentKey(), this.currentField()]).pipe(
      take(1),
    ).subscribe(([key, field]) => {
