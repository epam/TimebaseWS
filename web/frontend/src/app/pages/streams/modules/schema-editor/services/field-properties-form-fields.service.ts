...
import {
  DefaultTypeModel, FieldTypeModel,
...
export class FieldPropertiesFormFieldsService {
  ...
  fields(): Observable<FieldModel[]> {
    return combineLatest([
      ...
      map(([messages, type, field, allSchemaItems, defaultTypes]: [any, SchemaClassTypeModel, SchemaClassFieldModel, SchemaClassTypeModel[], DefaultTypeModel[]]) => {
        ...
        const classNames = allSchemaItems.filter(t => !t.isEnum && !t.isAbstract && !(t?._props?._isSelected)).map(t => t.name);
        let nameValues = [
          ...defaultTypeNames,
          ...enumNames,
        ];
  ...
  private getTypeField(
    ...
    const encodings = defaultTypes.filter(t => t.name === type.name && t.encoding).map(_type => _type.encoding);
  ...
  private getStaticValueField(messages, field: SchemaClassFieldModel, enums: SchemaClassTypeModel[]): FieldModel {
    ...
    switch (field.type.name) {
      case 'BOOLEAN':
        const values = [{value: 'true', label: 'true'}, {value: 'false', label: 'false'}];
        if (!required) {
          values.unshift({value: null, label: 'NULL'});
        }
        return {
  ...
  private getErrorsText(isName = false): (control: FormControl) => Observable<string> {
    return control => {
      return this.translateService.get('forms')
        .pipe(
          map(messages => {
            let stringHtml = '';
