...
import { filter, map, switchMap, take, takeUntil, tap, throttleTime, withLatestFrom } from 'rxjs/operators';
...
export class MonitorLogGridDataService {
  ...
  public getSubscription(url: string, dataObj: WSLiveModel) {
    ...
    this.appStore.pipe(select(getActiveOrFirstTab))
      .pipe(
        ...
        switchMap((activeTab: TabModel) => {
          ...
          if (activeTab.symbol) {
            httpUrl = `${encodeURIComponent(activeTab.stream)}/${encodeURIComponent(activeTab.symbol)}`;
          } else {
          ...
          return this.httpClient$
            .post<StreamDetailsModel[]>(httpUrl, params)
            /*.pipe(
             catchError((error) => {
        ...
        map(data => {
          this.rowsStore = data.map(row => new StreamDetailsModel(row)).sort((row1, row2) => {
        ...
        switchMap(() => this.wsService
          ...
          .pipe(
            ...
            map(([ws_data]) => ws_data),
            map(ws_data => JSON.parse(ws_data.body).map(row => new StreamDetailsModel(row))),
            map((data: StreamDetailsModel[]) => {
              ...
              return {
                data: this.rowsStore, newDataLength: data.length,
              };
            }),
            throttleTime(500),
