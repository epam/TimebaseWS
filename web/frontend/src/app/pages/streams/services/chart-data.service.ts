...
export class ChartDataService {
  ...
  public getNewData(
    ...
    isTail = false,
    pointsRate = 1,
  ): Observable<{response: ChartModel[] | boolean; type: string | null}> {
    return this.appStore.pipe(
      select(getActiveOrFirstTab),
      take(1),
      switchMap((activeTab) =>
        this.appStore.select(getAppSettings).pipe(
      ...
      switchMap(([activeTab, settings]: [TabModel, AppSettingsModel]) => {
        const type: ChartTypes =
          activeTab.filter.chart_type || (activeTab.chartType && activeTab.chartType[0]);
        const maxPoints = this.getMaxPointsCount(
          settings,
          isTail,
          pointsRate,
          activeTab.filter['levels'],
        );
        ...
        if (maxPoints >= 10) {
          ...
          response$ = this.httpClient.get<ChartModel[]>(
            `charting/${encodeURIComponent(activeTab.stream)}`,
            {
              params,
              headers: {
                customError: 'true',
