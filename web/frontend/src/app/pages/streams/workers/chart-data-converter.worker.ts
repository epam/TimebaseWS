...
addEventListener('message', ({data}) => {
  ...
  if (viewportDiff_changed) {
  ...
  } else if (chart_rawData && chart_rawData.lines && Object.keys(chart_rawData.lines).length) {
    for (const i in chart_rawData.lines) {
      if (chart_rawData.lines.hasOwnProperty(i)) {
        const new_line =
          chart_raw_newData &&
          chart_raw_newData.lines &&
          typeof chart_raw_newData.lines[i] === 'object'
            ? chart_raw_newData.lines[i]
            : {points: []};
        const merged_points = [];
        ...
        for (const point of [...chart_rawData.lines[i].points, ...new_line.points]) {
          if (
            !merged_points.find(
              (line_point) =>
                !!(line_point.time === point.time && line_point.value === point.value),
            )
          ) {
    ...
    [newLines, current_data_windowSize] = clearChartData(
      chart_rawData.lines,
      current_viewport,
      streamRange,
      tailsMultiplier,
    ...
    (data, line) => {
      line.points.forEach((point) => {
        ...
        data[0].dataPoints.push({
          x: time,
          y: [
            parseFloat(open as string),
            parseFloat(high as string),
            parseFloat(low as string),
            parseFloat(close as string),
          ],
    ...
    points.forEach((point, index) => {
      switch (lineKey) {
        case 'TRADES':
          dataPoints.push({
            x: point.time,
            y: parseFloat(point.value as string),
          });
          break;
        default:
          index++;
          const y = [parseFloat(point.askPrice as string), parseFloat(point.bidPrice as string)],
            newPoints: {
              x: number | string;
              y: (number | string)[];
              color?: string;
...
function eachLine<T>(
  chart: ChartModel,
  data: T[] = [],
  callback: (data: T[], line: ChartRawLine, lineKey: string) => void,
): ChartData {
...
function getChartData(chartModel: ChartModel, update?: boolean): ChartData {
  ...
  return eachLine(chartModel, data, (data, line, lineKey) => {
    ...
    points.forEach((point, index) => {
      index++;
      const y = parseFloat(point.value as string),
        newPoints = [
          {
            x: point.time,
            y: y,
  ...
  if (Object.keys(raw_lines).length) {
    for (const LINE_KEY in raw_lines) {
      if (raw_lines.hasOwnProperty(LINE_KEY)) {
        newLines[LINE_KEY] = {
          ...raw_lines[LINE_KEY],
          points: raw_lines[LINE_KEY].points.filter(
