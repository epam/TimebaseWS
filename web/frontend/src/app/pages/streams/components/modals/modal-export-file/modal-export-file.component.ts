...
import { ExportFilter, ExportFilterFormat, ExportTo } from '../../../../../shared/models/export-filter';
...
export class ModalExportFileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.tree$ = this.schemaService.getSchema(this.stream.key).pipe(
      map(({ types }) => {
    ...
    const fields$ = this.tree$.pipe(map(tree => {
      const fields: string[] = [];
      tree.forEach(type => {
    ...
    combineLatest([fields$, this.streamsService.range(this.stream.key)]).pipe(
      take(1),
    ...
    this.symbolsHeader$ = this.controlValue('symbols').pipe(
      switchMap((symbols: string[]) => !symbols?.length ?
    ...
    this.fieldsHeader$ = combineLatest([fields$, this.controlValue('fields')]).pipe(
      map(([fields, selected]: [string[], string[]]) => `${selected?.length || 0}/${fields.length}`),
    ...
    this.configHeader$ = this.form.valueChanges.pipe(
      ...
      map(([labels, values]) => labels.map((label, index) => `${label}: ${values[index]}`).join('; ')),
  ...
  export() {
    ...
    this.exportService.export(this.stream.key, filter).pipe(
  ...
  private getSymbols(term: string): Observable<string[]> {
    return this.symbolsService.getSymbols(this.stream.key, null, term);
  }

  private controlValue(controlName: string): Observable<unknown> {
    return this.form.get(controlName).valueChanges.pipe(startWith(this.form.get(controlName).value));
  }
}
