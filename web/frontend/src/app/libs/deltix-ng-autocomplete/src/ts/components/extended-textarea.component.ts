          ...
          <div class="autocomplete-dropdown" *ngIf="!disabled">
            <div class="autocomplete-dropdown-menu-wrapper" *ngIf="isShowDropdown()">
              <ul class="autocomplete-dropdown-menu">
                <li
                  *ngFor="let item of values; first as isFirst"
...
export class ExtendedTextareaComponent
  ...
  implements OnInit, ControlValueAccessor, OnDestroy, OnChanges
{
  ...
  public select(item: any, event: Event) {
    ...
    if (this.cursorPosition != null) {
      ...
      this.selectedText =
        this.selectedText.substr(0, pos.left) + v + this.selectedText.substr(pos.left + pos.length);
      setTimeout(() => {
  ...
  protected getInputPos(cursorPos: number): {left: number; length: number} {
    let offsetLeft = Utils.smartLastIndexOf(
      this.inputElement.value.substring(0, cursorPos),
      this.separator,
      this.literal,
    );
    if (offsetLeft === -1) {
