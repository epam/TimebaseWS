...
class Vizceral extends EventEmitter {
   ...
   */
  constructor(canvas, targetFramerate, graphSize = {}) {
    super();
    this.setGraphInitialSize(graphSize.width, graphSize.height);
  ...
  setView(viewArray = [], objectNameToHighlight) {
    ...
    if (!this.currentGraph || !isEqual(newGraph.graphIndex, this.currentGraph.graphIndex)) {
      ...
      } else if (difference && this.currentGraph.graphIndex.length === Math.abs(difference)) {
  ...
  setCurrentGraph(graph, redirectedFrom) {
    ...
    this.emit('viewChanged', {
      view: this.currentGraph.graphIndex,
  ...
  setGraphInitialSize(w, h) {
    graphWidth = w || graphWidth;
    graphHeight = h || graphHeight;
  }
