...
import { toDecimal, roundDecimalBy } from '@deltix/decimal-utils';
...
class TakeWhileInclusiveSubscriber extends Subscriber {
...
function isCreator(...creators) {
  ...
  return source => source.pipe(filter(v => {
    if (typeof v !== 'object') {
      return false;
    }
...
const select = selector => state$ => map(() => selector(state$.value));
...
const enhancePath = (path, reducer) => (state, action) => {
  const current = R.path(path, state);
  ...
  return R.assocPath(path, next, state);
};
...
const enhanceReducer = (part, reducer) => (state, action) => {
  ...
  if (nextStatePart !== statePart) {
    return R.assoc(part, nextStatePart, state);
  }
...
const createDecimalTicks = (min, max, ticks) => {
  ...
  const loverBoundary = toDecimal(stepSize * Math.floor(min.toNumber() / stepSize));
  const upperBoundary = toDecimal(stepSize * Math.ceil(max.toNumber() / stepSize)); // Build array
...
const formatPriceByPrecision = (precision, price) => {
  ...
  const value = roundDecimalBy(new Big(price), precision);
  return value instanceof Big ? value.toFixed() : value;
};
