...
public class SessionLoginSample {
    ...
    public static SessionDto login(String url, String sessionApiKey, String privateKey) {
        return loginConfirm(url, loginRequest(url, sessionApiKey), privateKey);
    }

    private static SuccessfulLoginAttemptDto loginRequest(String url, String apiKeyId) {
        ...
        try {
            ...
            return rest(HttpMethod.POST, url + LOGIN_ATTEMPT_PREFIX, entity, SuccessfulLoginAttemptDto.class)
                .getBody();
    ...
    private static SessionDto loginConfirm(String url, SuccessfulLoginAttemptDto loginAttempt, String privateKey) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        ...
        try {
            ...
            SuccessfulLoginConfirmationDto responseDto = rest(
                HttpMethod.POST, url + LOGIN_CONFIRM_PREFIX, entity, SuccessfulLoginConfirmationDto.class
            ).getBody();
    ...
    private static <T> ResponseEntity<T> rest(HttpMethod method, String url, HttpEntity<?> entity, Class<T> type) {
