...
public class BarConversionTransformation extends AbstractChartTransformation<BarElementDef, BarMessage> {
    ...
    private long stopBarTimestamp = Long.MIN_VALUE;
    ...
    protected void onNextPoint(BarMessage barMessage) {
        ...
        if (this.timestamp == Long.MIN_VALUE) {
            ...
            this.stopBarTimestamp = getTransformationStopBarTimestamp(barTimestamp, periodicity);
    ...
    private void flushPrev(long timestamp) {
        ...
        if (timestamp <= this.stopBarTimestamp) {
            return;
        }
